...
const ProjectCard = ({ imgUrl, title, description, date, environment, gitUrl, tag, result, mainWork }) => {
    ...
    <div className="border border-[#ADB7BE] rounded-xl overflow-hidden">
      ...
      >
        <div className="overlay items-center justify-center absolute top-0 left-0 w-full h-full bg-[#181818] bg-opacity-0 hidden group-hover:flex group-hover:bg-opacity-80 transition-all duration-500 ">
          <button onClick={openModal} className="h-14 w-14 mr-4 border-2 relative rounded-full border-[#ADB7BE] hover:border-white group/link">
        ...
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-70 z-50" onClick={handleOutsideClick}>
          <div ref={modalRef} className="text-black bg-white p-8 rounded-xl">
            <div>
              ...
              <h3 className="text-lg font-semibold mb-2">개발환경</h3>
              <ul className="list-disc ml-8 mb-4">
                {environment.map((data, index) => (
                  <li key={index}>{data}</li>
                ))}
              </ul>
              <h3 className="text-lg font-semibold mb-2">주요업무</h3>
              <ul className="list-disc ml-8 mb-4">
                {mainWork.map((data, index) => (
                  <li key={index}>{data}</li>
                ))}
              </ul>
              {result.length === 1 ? (
